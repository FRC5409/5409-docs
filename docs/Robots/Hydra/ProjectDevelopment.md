...
Properly managing **Current Limits** on your drivetrain is essential for protecting battery health and preventing brownouts. While the ```TunerConstants``` file generated by Phoenix Tuner includes **Stator Current Limits** to manage motor torque and wheel slip, it is critical to manually add **Supply Current Limits** to restrict the total amperage drawn from the battery. These supply limits act as a safety net during high-stress situations, such as heavy defensive pushing matches, ensuring the robot doesn't trip breakers even if the driver is at full throttle; however, these must be tuned carefully, as setting them too low will make the robot feel sluggish and limit the drive team’s maneuverability. For a deeper technical dive into optimizing these settings for TalonFX controllers, refer to the [CTRE Improving Performance With Current Limits](https://v6.docs.ctr-electronics.com/en/stable/docs/hardware-reference/talonfx/improving-performance-with-current-limits.html) documentation.
